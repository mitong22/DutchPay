...
function requireActiveGroup(viewer) {
...
function normalizeReceipt(input, members, currentReceipt, viewerMemberId) {
  ...
  return {
    ...
    items,
    image_key: currentReceipt?.image_key ?? null,
    input_method: currentReceipt?.input_method ?? "MANUAL",
    ocr_status: currentReceipt?.ocr_status ?? "NONE",
