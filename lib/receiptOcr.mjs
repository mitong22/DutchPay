...
// Teacher: AI가 만든 JSON도 입력 검증이 필요합니다. quantity × unit_price와 line_total이 다를 때 수량을 1로 바꾸는 경우를 계산하고, 인식된 원본 총액과 다시 합산한 총액을 어떻게 대조할지 논의해 보기.
...
// Teacher: 이 OCR은 서버가 외부 CLI 프로세스를 실행하고 결과 JSON 파일을 읽는 흐름입니다. 임시 폴더 → 실행 인자 → 시간 제한·중단 신호 → finally 삭제를 따라가고, 일반 fetch API 방식과 필요한 실행 환경을 비교해 보기.
