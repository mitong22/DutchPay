...
export function createActivatedTogetherState({
  expectedMemberCount,
  joinedMemberCount,
  activatedAt = new Date(),
}) {
  assertExpectedMemberCount(GROUP_MODE.TOGETHER, expectedMemberCount);
  assertJoinedMemberCount(joinedMemberCount);
  assertValidDate(activatedAt, "activatedAt");

  if (joinedMemberCount !== expectedMemberCount) {
    throw new Error(
      "A TOGETHER group can activate only when every expected member has joined.",
    );
  }
  ...
  return {
    ...
    status: GROUP_STATUS.ACTIVE,
    expected_member_count: expectedMemberCount,
    activated_at: new Date(activatedAt.getTime()),
  };
}
