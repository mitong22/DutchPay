...
사용자가 선택한 사진을 미리 확인한 뒤 사진을 참고하여 가게명, 메뉴명, 수량과 금액을 직접 입력한다.
...
현재 목업에서는 촬영한 이미지를 미리 확인한 뒤 직접 입력 Form으로 이동한다.
...
현재 목업에서는 확인을 위해 축소한 이미지를 브라우저 `localStorage`에 저장한다. 실제 구현에서는 이미지 바이너리를 MongoDB 문서가 아니라 객체 스토리지에 저장한다.
