...
// Teacher: OCR 글자 상자의 중심 좌표를 구해 y로 줄을 묶고 x로 정렬합니다. 글자 4개의 좌표를 종이에 찍어 tolerance·reduce의 중간값을 추적하고, AI에게 명시적인 for문으로 풀어 달라고 요청해 보기.
...
// Teacher: 메뉴·수량·금액을 위치와 정규식으로 추정하므로 영수증 형식이 바뀌면 누락될 수 있습니다. 기존 OCR 테스트의 정상·누락 예를 따라가고, 추정 결과와 결제 총액을 사용자에게 대조시키는 지점을 확인해 보기.
