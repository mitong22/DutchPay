...
function buildItems(menuInputs, existingItems = []) {
  ...
  return menuInputs.map((menuInput) => {
    ...
    // Teacher: 메뉴 금액과 참여자 집합이 같으면 저장된 나머지 담당자를 유지합니다. 이름만 수정했는데 10원을 더 낼 사람이 바뀌면 안 되기 때문입니다. haveSameMemberIds가 배열 순서까지 비교하지 않는 이유와, 금액 또는 참여자가 바뀐 경우를 각각 테스트로 확인해 보세요.
    const settlementInputsUnchanged =
      existingItem !== null &&
      Array.isArray(existingItem.remainder_recipient_member_ids) &&
...
function buildPayments({
  ...
  // Teacher: flatMap 안의 map은 메뉴 수 × 각 메뉴 참여자 수만큼 payment를 만듭니다. 이어지는 삼항 연산은 본인 결제·기존 상태 유지·unpaid 초기화를 구분합니다. AI에게 중첩 for문과 if문으로 풀어 쓰게 하고, 금액·참여자·결제자 변경 시 기존 paid가 언제 초기화되는지 비교해 보세요.
