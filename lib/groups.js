...
// Teacher: 함수를 인자로 받아 MongoDB session을 넘기는 공통 실행기입니다. createGroup의 콜백을 이 위치에 펼쳐 실행 순서를 쓰고, 여러 문서가 함께 저장·취소되는 범위와 finally의 endSession을 확인해 보기.
...
// Teacher: Set으로 허용 참여자·기존 메뉴·사용한 메뉴 ID를 따로 관리합니다. 새 영수증과 수정 영수증 하나씩으로 재사용되는 ID를 추적하고, AI에게 검증 단계를 명시적인 반복문으로 풀게 해 보기.
...
// Teacher: flatMap은 메뉴별 참여자 배열을 한 배열로 펼칩니다. 메뉴 2개 × 참여자 2명 예제로 payment 건수를 세고, 이전 상태를 찾는 세 ID 조합과 최종 송금 관계가 같은 것인지 비교해 보기.
...
// Teacher: 현재 완료 처리는 settlement_completed_at을 기록합니다. 실제 계좌 송금이나 별도 정산 결과 저장을 하는지 함수 끝까지 확인하고, 화면의 완료 표시·영수증 수정 제한·송금 실행을 구분해 보기.
