...
JPEG, PNG, WebP 이미지를 최대 10MB까지 받는다. 서버는 현재 사용자가 활성화된 모임의 멤버인지 확인한 뒤 이미지를 로컬 디스크의 `.local/receipts`에 저장한다.

저장한 이미지는 로컬 `codex-cli`의 이미지 입력과 JSON 출력 스키마를 사용해 분석한다. 분석 결과는 `receipts.items[]`와 같은 `menu_name`, `quantity`, `unit_price`, `line_total` 형식으로 정규화한다.

서버를 실행하는 로컬 계정에서 `codex login status`가 로그인 상태여야 한다. 실행 파일은 현재 `PATH`, `CODEX_INSTALL_DIR`, 독립 실행형 CLI 기본 위치, macOS의 ChatGPT/Codex 앱 내장 위치에서 자동 탐색한다. 이 위치들 밖에 설치했다면 `CODEX_CLI_PATH`에 절대 경로를 설정한다.

OCR 호출은 임시 세션과 읽기 전용 샌드박스를 사용하고 shell 및 web search를 끈다. 앱의 DB·인증 환경변수도 Codex 작업에 전달하지 않는다.
...
서버 로컬 디스크에 이미지 저장
↓
codex-cli OCR 및 JSON 스키마 검증
↓
메뉴·수량·개당 금액이 선입력된 확인 화면
↓
사용자 확인 및 수정 후 MongoDB 저장
```
...
현재 로컬 기능 목업에서는 DB에 영향을 주지 않는 단일 테스트 계정 `1234 / 1234`와 HttpOnly Cookie를 사용한다. 로그인 화면에 테스트 자격 증명을 함께 표시하며, 실제 계정 단계에서 이 목업 세션을 Better Auth 로그인으로 교체한다.
...
루트 경로 `/`에 처음 접속하면 로그인 화면을 표시한다. 로그인한 총대에게는 정산 대시보드를 먼저 보여주고, **새 정산 시작**을 선택했을 때 모임 생성 Stepper로 이동한다. 로그인하지 않은 비회원이 유효한 초대 URL로 접속한 경우에는 로그인 화면을 거치지 않고 초대 참여 화면을 표시한다.
...
# 선택 사항: 자동 탐색되지 않는 위치에 설치했을 때만 지정
...
image_key
→ 로컬 디스크에 저장한 영수증 이미지의 모임별 상대 키 또는 null
...
이미지 바이너리는 MongoDB에 넣지 않고 프로젝트의 `.local/receipts` 아래에 저장한다. MongoDB에는 모임별 상대 경로인 `image_key`만 저장한다. 메뉴 확인 전에 창을 닫은 미확정 이미지는 현재 자동 삭제하지 않는다.
...
현재 로컬 기능 목업은 첫 로그인 시 확인용 `정산 중` 모임 1개와 `정산 완료` 모임 1개를 브라우저 저장소에 멱등하게 추가한다. 기존에 직접 만든 모임은 삭제하거나 덮어쓰지 않는다.
