...
import { createUserIdCandidates } from "@/lib/utils/user-id.mjs";
...
export async function listOwnedGroups(userId) {
  const createdByCandidates = createUserIdCandidates(userId);

  return db
    .collection("expense_group")
    .find(
      { created_by: { $in: createdByCandidates } },
      {
        projection: {
          name: 1,
          mode: 1,
...
export async function getGroupWorkspaceData(groupId, selectedReceiptId) {
  const [members, storedReceipts, invites] = await Promise.all([
    db
      .collection("group_member")
  ...
  const receipts = storedReceipts.map(normalizeReceiptForRead);
...
export async function getReceiptEditorData(groupId, receiptId = null) {
  const [members, storedReceipt] = await Promise.all([
    db
      .collection("group_member")
      .find({ group_id: groupId })
      .sort({ nickname: 1, _id: 1 })
  ...
  return {
    members,
    receipt: normalizeReceiptForRead(storedReceipt),
  };
}
