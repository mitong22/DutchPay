...
// Teacher: JSON 왕복은 깊은 복사처럼 보이지만 Date·ObjectId 등의 표현도 바꿉니다. 변환 전후 타입을 적고, Client에 필요한 필드만 명시적으로 변환하거나 Server에서 원본 문서를 표시하는 대안과 비교해 보기.
...
// Teacher: work는 실행할 함수를 인자로 받은 것입니다. 모임 생성 콜백을 여기 펼쳐 쓰며 DB session의 전달 경로를 추적하고, 인증 session과 다르다는 점 및 finally가 연결 세션을 정리하는 이유를 확인해 보기.
...
export async function joinGroup(token, nicknameValue, userId) {
  return transaction(async (session) => {
    ...
    // Teacher: 이 브랜치는 한 사람당 한 번 쓰는 초대권입니다. used_at:null 조건이 있는 갱신과 트랜잭션을 따라가며, 같은 링크로 동시에 두 명이 들어오면 누가 참여 가능한지 설명해 보기.
...
function normalizeReceipt(input, members, currentReceipt, viewerMemberId) {
  ...
  // Teacher: 이 브랜치의 영수증 결제자는 서버가 확인한 본인으로 고정합니다. 화면의 readOnly 입력을 바꿔 요청하더라도 이 값이 결정되는 경로를 확인하고, 다른 브랜치의 결제자 선택 방식과 구분해 보기.
...
export async function saveReceipt(groupId, input, viewer) {
  ...
  const existingPayments = currentReceipt
    ? await db
        ...
        .toArray()
    : [];
  // Teacher: 메뉴·부담자·결제자 ID 조합으로 기존 paid 상태를 찾습니다. 영수증 금액만 바뀌고 ID는 같을 때 이전 완료 상태가 유지되는지 확인하고, 수정 후 어떤 상태를 다시 확인해야 할지 논의해 보기.
